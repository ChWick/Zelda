...
protected:
	...
	Ladder *m_pLadder;                              //!< the ladder on which the player is climbing or null
	int m_iInitialClimbingPosition;                 //
    ...
    CPlayerTools &m_Tools;                           //!< handle the different tools, sword, bow, lamp, ...
public:
	...
	const CPlayerTools &getTools() const {return m_Tools;}
    ...
    /** This function will be called out of the ladder object, when an person interacts on collision
      ...
      * @param fPosOnLadder the initial value for the position on the ladder
      */
