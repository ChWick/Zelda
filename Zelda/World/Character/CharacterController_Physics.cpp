...
class btKinematicClosestNotMeRayResultCallback
 ...
 public:
  explicit btKinematicClosestNotMeRayResultCallback(btCollisionObject* me)
 ...
 */
btVector3 CharacterControllerPhysics::computeReflectionDirection(
 ...
 */
btVector3 CharacterControllerPhysics::parallelComponent(
    const btVector3& direction,
    const btVector3& normal) {
 ...
 */
btVector3 CharacterControllerPhysics::perpindicularComponent(
    const btVector3& direction,
    const btVector3& normal) {
...
CharacterControllerPhysics::CharacterControllerPhysics(
    CCharacterControllerPhysicsListener &listener,
...
bool CharacterControllerPhysics::recoverFromPenetration(
  ...
  for (int i = 0;
       i < m_ghostObject->getOverlappingPairCache()->getNumOverlappingPairs();
       i++) {
    ...
    for (int j = 0; j < m_manifoldArray.size(); j++) {
      ...
      for (int p = 0; p < manifold->getNumContacts(); p++) {
        ...
        if (dist < 0.0) {
          ...
          m_currentPosition +=
              pt.m_normalWorldOnB * directionSign * dist * btScalar(0.2);
          penetration = true;
        } else {
          // printf("touching %f\n", dist);
        }
...
void CharacterControllerPhysics::stepUp(btCollisionWorld* world)
{
  ...
  m_targetPosition = m_currentPosition + getUpAxisDirections()[m_upAxis]
  ...
  if (callback.hasHit()) {
    // Only modify the position if the hit was a slope
    // and not a wall or ceiling.
...
void CharacterControllerPhysics::updateTargetPositionBasedOnCollision(
  ...
  if (movementLength > SIMD_EPSILON) {
    ...
    btVector3 reflectDir = computeReflectionDirection(movementDirection,
                                                      hitNormal);
...
void CharacterControllerPhysics::stepForwardAndStrafe(
    btCollisionWorld* collisionWorld,
  ...
  while (fraction > btScalar(0.01) && maxIter-- > 0) {
    ...
    if (callback.hasHit()) {
      ...
      if (pOther->getBroadphaseHandle()->m_collisionFilterGroup == COL_BORDER) {
        ...
        btScalar cosangle =
            callback.m_hitNormalWorld.dot(m_normalizedDirection);
        if (cosangle > -0.95f) {
      ...
      // m_currentPosition.setInterpolate3(m_currentPosition,
      // m_targetPosition, callback.m_closestHitFraction);
...
void CharacterControllerPhysics::stepDown(btCollisionWorld* collisionWorld,
                                          btScalar dt) {
  ...
  btScalar downVelocity =
      (m_verticalVelocity < 0.f ? -m_verticalVelocity : 0.f) * dt;
  ...
  btVector3 step_drop =
      getUpAxisDirections()[m_upAxis] * (m_currentStepOffset + downVelocity);
  ...
  while (1) {
    ...
    // set double test for 2x the step drop,
    // to check for a large drop vs small drop
    ...
    if (m_useGhostObjectSweepTest) {
      ...
      if (!callback.hasHit()) {
        // test a double fall height, to see if the character should
        // interpolate it's fall (full) or not (partial)
        m_ghostObject->convexSweepTest(
            m_convexShape, start, end_double, callback2,
            collisionWorld->getDispatchInfo().m_allowedCcdPenetration);
      }
    } else {
      collisionWorld->convexSweepTest(
      ...
      if (!callback.hasHit()) {
        // test a double fall height, to see if the character should
        // interpolate it's fall (large) or not (small)
        collisionWorld->convexSweepTest(
            m_convexShape, start, end_double, callback2,
            collisionWorld->getDispatchInfo().m_allowedCcdPenetration);
      }
    ...
    if (downVelocity2 > 0.0 && downVelocity2 < m_stepHeight
        && has_hit == true && runonce == false
        && (m_wasOnGround || !m_wasJumping)) {
      // redo the velocity calculation when falling a small amount,
      // for fast stairs motion
      // for larger falls, use the smoother/slower interpolated movement
      // by not touching the target position
  ...
  if (callback.hasHit() || runonce == true) {
    ...
    btScalar fraction =
        (m_currentPosition.getY() - callback.m_hitPointWorld.getY()) / 2;
    ...
    if (bounce_fix == true) {
      if (full_drop == true) {
        ...
        // due to errors in the closestHitFraction variable when used with
        // large polygons, calculate the hit fraction manually
        m_currentPosition.setInterpolate3(m_currentPosition,
                                          m_targetPosition,
                                          fraction);
      }
    ...
    if (bounce_fix == true) {
      ...
      if (downVelocity > m_fallSpeed && (m_wasOnGround || !m_wasJumping)) {
        ...
        downVelocity = m_fallSpeed;
        step_drop = getUpAxisDirections()[m_upAxis]
            * (m_currentStepOffset + downVelocity);
        m_targetPosition -= step_drop;
      }
    }
...
void CharacterControllerPhysics::reset(btCollisionWorld* collisionWorld) {
  ...
  while (cache->getOverlappingPairArray().size() > 0) {
    cache->removeOverlappingPair(
        cache->getOverlappingPairArray()[0].m_pProxy0,
...
void CharacterControllerPhysics::preStep(btCollisionWorld* collisionWorld) {
  ...
  /* printf("m_targetPosition=%f,%f,%f\n",m_targetPosition[0],
     m_targetPosition[1],m_targetPosition[2]);*/
}
...
void CharacterControllerPhysics::playerStep(btCollisionWorld* collisionWorld,
  ...
  if (m_verticalVelocity < 0.0
     && btFabs(m_verticalVelocity) > btFabs(m_fallSpeed)) {
...
void CharacterControllerPhysics::jump(bool bForce) {
  ...
  up.normalize();
  btScalar magnitude = (btScalar(1.0)
                        / m_rigidBody->getInvMass()) * btScalar(8.0);
