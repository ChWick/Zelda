#ifndef _PAUSE_TYPES_HPP_
#define _PAUSE_TYPES_HPP_

enum EPauseTypes {
  PAUSE_NONE              = 0,

  PAUSE_WORLD_UPDATE      = 1,
  PAUSE_WORLD_RENDER      = 2,

  PAUSE_ATLAS_UPDATE      = 4,
  PAUSE_ATLAS_RENDER      = 8,

  PAUSE_ALL_WORLD_UPDATE  = PAUSE_WORLD_UPDATE | PAUSE_ATLAS_UPDATE,
  PAUSE_ALL_WORLD_RENDER  = PAUSE_WORLD_RENDER | PAUSE_ATLAS_RENDER,
  PAUSE_ALL_WORLD         = PAUSE_ALL_WORLD_RENDER | PAUSE_ALL_WORLD_UPDATE,

  PAUSE_ALL_ATLAS         = PAUSE_ATLAS_RENDER | PAUSE_ATLAS_UPDATE,

  PAUSE_ALL               = 4095,
};

#endif // _PAUSE_TYPES_HPP_
